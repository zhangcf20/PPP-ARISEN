...
//2020.10.30 23z
    ...
    {
        double ex_cf[3] = {0}, ey_cf[3] = {0}, ez_cf[3] = {0};
        //取太阳和卫星连线的单位向量，从而求出ey
        double vectSunSat[3] = {0},lenSunSta = 0.0;
        for(int i = 0; i < 3; i++)
        {
            vectSunSat[i] = m_sunpos[i] - StaPos[i];
            ez_cf[i] = -StaPos[i]/lenStaPos;
        }
        lenSunSta = qSqrt(m_CmpClass.InnerVector(vectSunSat,vectSunSat));
        double nsun[3] = {0};
        for(int i = 0; i < 3; i++)
            nsun[i] = vectSunSat[i]/lenSunSta;

        m_CmpClass.OutVector(ez_cf,nsun,ey_cf);
        double leney = 0.0;
        leney = qSqrt(m_CmpClass.InnerVector(ey_cf,ey_cf));
        for(int i = 0; i < 3; i++)
            ey_cf[i] = ey_cf[i]/leney;

        m_CmpClass.OutVector(ey_cf,ez_cf,ex_cf);
        double lenex = 0.0;
        lenex = qSqrt(m_CmpClass.InnerVector(ex_cf,ex_cf));

        for(int cf_i = 0; cf_i < 3; cf_i++)
            ex[cf_i] = ex_cf[cf_i],ey[cf_i] = ey_cf[cf_i],ez[cf_i] = ez_cf[cf_i];
    }
    ...
    //double sat2recV[3] = {StaPos[0] - RecPos[0],StaPos[1] - RecPos[1],StaPos[2] - RecPos[2]};
