...
{// Preserving the results of GPT2
    ...
    double e;// water vapour pressure in hPa
    double undu;//geoid undulation in m
    double la;// water vapour decrease factor la
    double Tm;// mean temperature Tm
    double ah;//hydrostatic mapping function coefficient at zero height (VMF3)
...
{// Store GRD file data for calculation
    ...
    double ahgrid[5];//hydrostatic mapping function coefficient, dimensionless
    double awgrid[5];// wet mapping function coefficient, dimensionless
    double lagrid[5];//water vapor decrease factor, dimensionless
    double Tmgrid[5];//mean temperature in Kelvin
    double Gn_h_grid[5];//hydrostatic north gradient in m
    double Ge_h_grid[5];//hydrostatic east gradient in m
    double Gn_w_grid[5];//wet north gradient in m
    double Ge_w_grid[5];//wet east gradient in m
}GrdFileVar3;
...
public:
    ...
    void getVMF1Parm(double ah,double aw,double E,double Lat,double Lon,double H,int TDay,double &md,double &mw);// Calculating VMF1 dry weight (md) and wet weight (mw) projection function E as elevation angle (rad) H as positive height (m) T as annual accumulated day Lat as geodetic latitude (rad north latitude greater than zero south latitude less than zero) awbw from GPT 2 model estimation
    ...
    //2021.06.01 23z
    void VMF1model(double ah,double aw,double E,double Lat,double Lon,double H,int doy,double &md,double &mw);
    void VMF3model(double ah,double aw,double E,double Lat,double Lon,double H,int doy,double &md,double &mw);
    GPT3Result GPT3model(double doy,double dlat,double dlon,double hell,double it = 0);
    double GPT3saszd(double MJD, int TDay, double E, double *pBLH, double *mf = NULL, double *ZPD = NULL, double *tZHD = NULL);
...
private:
    ...
    //2021.06.03 23z
    void readGrd3File(QString grdFileName);
#ifdef EIGEN_CORE_H
...
private:
    ...
    QVector< GrdFileVar3 > m_allGrdFile3;
	QString tempLine;
	bool isReadAllData;
    bool isGPT;// Determine whether it is a GPT model: 1 or choose GPT:0
