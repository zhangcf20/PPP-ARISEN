...
bool QRTWrite2File::setSaveFloder(QString fload_path, QString obsinfo)
{
    ...
    QString ofn = obsinfo + "position.txt";
    tempallSaveFiles.append(ofn);
    ofn = obsinfo + "bad_satellites.txt";
    tempallSaveFiles.append(ofn);
    ofn = obsinfo + "modelling.txt";
    tempallSaveFiles.append(ofn);
    ofn = obsinfo + "ZTD_Clock.txt";
    tempallSaveFiles.append(ofn);

    //    tempallSaveFiles << "position.txt" << "bad_satellites.txt" << "Satellite_info.ppp" << "ZTD_Clock.txt";
...
bool QRTWrite2File::writeRecivePos2Txt(const RecivePos oneRecivePos, MatrixXd *Qmat)
{
    ...
    if(oneRecivePos.UTCtime.epochNum == 0)
    {//23z
...
{
    QString ofn = m_ofn + "modelling.txt";
    int _flag = findFilenameFlag(ofn);
    if(_flag == -1) return false;
    QTextStream &saveFileOut = m_QTextStreamVct[_flag];
...
{
    int _flag = findFilenameFlag("ZTD_Clock.txt");
    if(_flag == -1) return false;
...
{
    int _flag = findFilenameFlag("bad_satellites.txt");
    if(_flag == -1) return false;
