                                     ...
                                     double *pXYZ,double *EA,double *psunpos, double *pmoonpos,double gmst,QString StationName)
{
    ...
    isGetPos = false;
    //Two differences greater than 0.3s are considered to replace the epoch to recalculate the sun coordinates
    //(a single epoch does not repeatedly calculate the sun coordinates)
    if (qAbs(m_SecondFlag - (Hours*3600+Minuts*60+Seconds)) > 0.3)
    {
        if(psunpos&&pmoonpos)
            setSunMoonPos(psunpos,pmoonpos,gmst);
        else
            m_cmpClass.getSunMoonPos(Year,Month,Day,Hours,Minuts,Seconds,m_sunpos,m_moonpos,&m_gmst);
        //An epoch only needs to be calculated once, no need to calculate multiple times
