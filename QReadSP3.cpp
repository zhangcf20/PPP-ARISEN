...
{//Lagrangian interpolation, here and after the selection - a total of 8 points for interpolation. GPST-satellite launch time within seconds of the week
    ...
    double sumDX[2] = {0},sumDY[2] = {0},sumDZ[2] = {0};
    //Satellite coordinates of +-0.5s before and after interpolation are used to simultaneously determine the speed
...
void QReadSP3::getPrcisePoint(int PRN, char SatType, double GPST, double *pXYZ, double *pdXYZ, double *pSp3Clk)
{
    ...
    //According to rtklib modification, use 11-order Neville's algorithm to calculate satellite coordinates 2021.01.05 23z

    lagrangeMethod(PRN,SatType,GPST,pXYZ,pdXYZ, pSp3Clk);

    Neville(PRN,SatType,GPST,pXYZ,pdXYZ, pSp3Clk);
...
//Neville 2021.01.05 23z
void QReadSP3::Neville(int PRN, char SatType, double GPST, double *pXYZ, double *pdXYZ, double *pSp3Clk)
{
    //Match time
    int lengthEpoch = m_allEpochData.length();
    int GPSTflag = m_lastGPSTimeFlag;
    ...
    if (ACCELERATE) m_lastGPSTimeFlag = GPSTflag;

    //Get 11 points, if not enough, get as many points as possible
    int num = 0;
    QVector< double > px; QVector< double > py; QVector< double > pz; QVector< double > pclk; QVector< int > pgpst;
    QVector< double > pox; QVector< double > poy; QVector< double > dltat;
    if(GPSTflag >= 5 && (lengthEpoch - GPSTflag) >= 5)
    {
        num = 11;
    }
    else if(GPSTflag >= 5 && (lengthEpoch - GPSTflag) < 5)
    {
        num = lengthEpoch - GPSTflag + 6;
    }
    else if(GPSTflag < 5 && (lengthEpoch - GPSTflag) >= 5)
    {
        num = GPSTflag + 6;
    ...
    for (int i = 0; i < num; i++)
    {
        if(SatType == 'G')
        {
            SP3Data epochData = m_allEpochData.at(GPSTflag - 5 + i);
            pox.append(epochData.MatrixDataGPS(PRN - 1,1));
            poy.append(epochData.MatrixDataGPS(PRN - 1,2));
            pz.append(epochData.MatrixDataGPS(PRN - 1,3));
            pclk.append(epochData.MatrixDataGPS(PRN - 1,4));
            pgpst.append(epochData.GPSTime);
            dltat.append(pgpst.at(i) - GPST);
        }
        if(SatType == 'E')
        {
            SP3Data epochData = m_allEpochData.at(GPSTflag - 5 + i);
            pox.append(epochData.MatrixDataGalieo(PRN - 1,1));
            poy.append(epochData.MatrixDataGalieo(PRN - 1,2));
            pz.append(epochData.MatrixDataGalieo(PRN - 1,3));
            pclk.append(epochData.MatrixDataGalieo(PRN - 1,4));
            pgpst.append(epochData.GPSTime);
            dltat.append(pgpst.at(i) - GPST);
        }
        if(SatType == 'C')
        {
            SP3Data epochData = m_allEpochData.at(GPSTflag - 5 + i);
            pox.append(epochData.MatrixDataBDS(PRN - 1,1));
            poy.append(epochData.MatrixDataBDS(PRN - 1,2));
            pz.append(epochData.MatrixDataBDS(PRN - 1,3));
            pclk.append(epochData.MatrixDataBDS(PRN - 1,4));
            pgpst.append(epochData.GPSTime);
            dltat.append(pgpst.at(i) - GPST);
        }
        if(SatType == 'R')
        {
            SP3Data epochData = m_allEpochData.at(GPSTflag - 5 + i);
            pox.append(epochData.MatrixDataGlonass(PRN - 1,1));
            poy.append(epochData.MatrixDataGlonass(PRN - 1,2));
            pz.append(epochData.MatrixDataGlonass(PRN - 1,3));
            pclk.append(epochData.MatrixDataGlonass(PRN - 1,4));
            pgpst.append(epochData.GPSTime);
            dltat.append(pgpst.at(i) - GPST);
        }
    ...
    double sinl = 0.0, cosl = 0.0;
    for(int cf_i = 0; cf_i < num; cf_i++)
    {
        sinl = sin(OMGE*dltat.at(cf_i));
        cosl = cos(OMGE*dltat.at(cf_i));
        px.append(cosl*pox.at(cf_i) - sinl*poy.at(cf_i));
        py.append(sinl*pox.at(cf_i) + cosl*poy.at(cf_i));
    }


    for(int cf_j = 1; cf_j < num; cf_j++)
    {
        for(int cf_i = 0; cf_i < (num - cf_j); cf_i++)
        {
            px[cf_i] = (dltat.at(cf_i + cf_j)*px.at(cf_i) - dltat.at(cf_i)*px.at(cf_i + 1))/(dltat.at(cf_i + cf_j) - dltat.at(cf_i));
            py[cf_i] = (dltat.at(cf_i + cf_j)*py.at(cf_i) - dltat.at(cf_i)*py.at(cf_i + 1))/(dltat.at(cf_i + cf_j) - dltat.at(cf_i));
            pz[cf_i] = (dltat.at(cf_i + cf_j)*pz.at(cf_i) - dltat.at(cf_i)*pz.at(cf_i + 1))/(dltat.at(cf_i + cf_j) - dltat.at(cf_i));
