 ...
 * reference: none (it just data processing experience).
 */
bool QualityCtrl::VtPVCtrl_Filter_LC(MatrixXd mat_B, VectorXd vec_L, VectorXd vec_X, VectorXd &del_flag , int sat_len, QVector< double > LP_threshold)
...
//    bool cf = m_matrix.writeCSV("/home/zcf/Documents/Balabala/data/B.csv", mat_B);
//    cf = m_matrix.writeCSV("/home/zcf/Documents/Balabala/data/X.csv", vec_X);
//    cf = m_matrix.writeCSV("/home/zcf/Documents/Balabala/data/L.csv", vec_L);
...
//    }
    ...
    for(int i = 0;i < sat_len; i++)
    {
        if(LP_threshold.length() != 0)
        {
            L_threshold = LP_threshold.at(i);P_threshold = L_threshold*130;
        }
...
void QualityCtrl::IGG3Algorithm(MatrixXd mat_B, VectorXd vec_L, VectorXd vec_X, MatrixXd &mat_P, int sat_len)
{
//计算V=BX-L
    VectorXd vec_V;
    vec_V = mat_B * vec_X - vec_L;
...
//IGG3判断
    double vigg=0.0;
    double k0=1.5,k1=3.0;
    for(int j = 0; j < sat_len; j++)
    {
        //标准化
        vigg = qAbs(vec_V[j]/Zgama);

        if(vigg > k1) mat_P(j,j)=1e-10;
        if(vigg <= k1 && vigg > k0)
        {
            double iggo = (k0/vigg)*((k1-vigg)/(k1-k0))*((k1-vigg)/(k1-k0));
            mat_P(j,j) = mat_P(j,j)*iggo;
        }
    }
}
